...
def adjust_first_seen(apps, schema_editor):
    ...
    sub_q = models.Subquery(
        System.objects.filter(system_id=models.OuterRef("system_id"))
        .values("system_id")
        .annotate(min__created=models.Min("created"))
        .values("min__created")
    )
    System.objects.annotate(min__created=sub_q).update(first_seen=models.F("min__created"))
